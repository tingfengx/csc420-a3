...
\begin{itemize}
	...
	\item \texttt{get\_gradient\_magnitude\_hist} gets the gradients in both $x, y$ directions, gradient magnitudes using $L_2$ norm, directions of the gradients, and the required $m \times n \times 6$ histogram tensor. It has a default $\tau = 8$, which you can customize. 
...
\end{itemize}
Figures \ref{fig:1hog}, \ref{fig:2hog}, and \ref{fig:3hog} are visualizations of the histogram array on the three given images. I thresholded the extreneously large values inside the histogram array to make the plots look nicer. (before this thresholding, there are some extremely long vectors shown, and they ruined the aesthetics of the plot. ) 
\newline \newline
\noindent {\Large \textbf{You can find my \texttt{txt} files under the \texttt{Q4} folder. They are such that \texttt{1.jpg -> 1.txt}, \texttt{2.jpg -> 2.txt}, and \texttt{3.jpg -> 3.txt}. }}

\begin{figure}
	\center\includegraphics[width=0.8\textwidth]{figs/quiver_1}
	\caption{\label{fig:1hog} HOG visualization over image 1.jpg}
\end{figure}
\begin{figure}
	\center\includegraphics[width=0.8\textwidth]{figs/quiver_2}
	\caption{\label{fig:2hog} HOG visualization over image 2.jpg}
\end{figure}
\begin{figure}
	\center\includegraphics[width=0.8\textwidth]{figs/quiver_3}
	\caption{\label{fig:3hog} HOG visualization over image 3.jpg}
\end{figure}
